...
export default function InfrastructureMap() {
  ...
  const nodes: Node[] = initialNodes.map((node) => {
    const isAffected = incidents.some(
      (i: any) =>
        i.severity !== "SAFE" &&
        i.service === String(node.data?.label)
    );
    ...
    <div className="flex flex-col gap-6 h-full p-6 relative">
      <div className="flex items-center justify-between relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-foreground text-glow-cyan">
              INFRASTRUCTURE INTELLIGENCE MAP
            </h1>

            <p className="text-xs font-mono tracking-widest text-brand-cyan/60 uppercase">
              Live service topology and dependency mapping
            </p>
          </div>
        </div>
      </div>
